...
landmark_positions = {'NOSE': 0, 'LEFT_EYE_INNER': 1, 'LEFT_EYE': 2, 'LEFT_EYE_OUTER': 3, 'RIGHT_EYE_INNER': 4, 'RIGHT_EYE': 5, 'RIGHT_EYE_OUTER': 6, 'LEFT_EAR': 7, 'RIGHT_EAR': 8, 'MOUTH_LEFT': 9, 'MOUTH_RIGHT': 10, 'LEFT_SHOULDER': 11, 'RIGHT_SHOULDER': 12, 'LEFT_ELBOW': 13, 'RIGHT_ELBOW': 14, 'LEFT_WRIST': 15,
                      'RIGHT_WRIST': 16, 'LEFT_PINKY': 17, 'RIGHT_PINKY': 18, 'LEFT_INDEX': 19, 'RIGHT_INDEX': 20, 'LEFT_THUMB': 21, 'RIGHT_THUMB': 22, 'LEFT_HIP': 23, 'RIGHT_HIP': 24, 'LEFT_KNEE': 25, 'RIGHT_KNEE': 26, 'LEFT_ANKLE': 27, 'RIGHT_ANKLE': 28, 'LEFT_HEEL': 29, 'RIGHT_HEEL': 30, 'LEFT_FOOT_INDEX': 31, 'RIGHT_FOOT_INDEX': 32}
...
# This list was used previously but due to lengthy remarks was inefficient
# remarks_list = [
#     'Your neck is not correctly positioned',
#     'Your left shoulder is not correctly positioned',
#     'Your right shoulder is not correctly positioned',
#     'Your left elbow is not correctly positioned',
#     'Your right elbow is not correctly positioned',
#     'Your left thigh is not correctly positioned',
#     'Your right thigh is not correctly positioned',
#     'Your left knee is not correctly positioned',
#     'Your right knee is not correctly positioned',
...
def generate_remarks(current_feed_landmarks, pose_name, side_faced_pose):

    # print(current_feed_landmarks)

    correct_posture_range = getattr(pose_data, pose_name)

    try:
        correctness_checklist = generate_correctness_checklist(
            correct_posture_range, current_feed_landmarks)
    ...
    elif correctness_checklist.count(0) > len(correctness_checklist)/2:
        feedback.append('You are not correctly visible in camera')
        return feedback
